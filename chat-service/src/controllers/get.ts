import { IConversationDocument, IMessageDocument } from '@sidharrrthnix/ms-shared-package';
...
const conversationList = async (req: Request, res: Response): Promise<void> => {
  ...
  res.status(StatusCodes.OK).json({ message: 'Chat conversation list', messages });
};
