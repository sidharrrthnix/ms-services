...
import { addDataToIndex, deleteIndexedData, getIndexedData, updateIndexedData } from '@gig-service/elasticsearch';
import { GigModel } from '@gig-service/models/gig.schema';
import { publishDirectMessage } from '@gig-service/queues/gigs.producer';
import { gigChannel } from '@gig-service/server';
import { Exchange, IRatingTypes, IReviewMessageDetails, ISellerDocument, ISellerGig, RoutingKey } from '@sidharrrthnix/ms-shared-package';
...
const createGig = async (gig: ISellerGig): Promise<ISellerGig> => {
  ...
  if (createdGig) {
    ...
    await publishDirectMessage(
      gigChannel,
      Exchange.SellerUpdate,
      RoutingKey.SellerUpdate,
...
export { createGig, deleteGig, getGigById, getSellerGigs, getSellerPausedGigs, seedData, updateActiveGigProp, updateGig, updateGigReview };
