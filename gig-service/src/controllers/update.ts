...
import { gigUpdateSchema } from '@gig-service/schemes/gig';
import { updateActiveGigProp, updateGig } from '@gig-service/services/gig.service';
import { BadRequestError, isDataURL, ISellerGig } from '@sidharrrthnix/ms-shared-package';
...
const gigUpdate = async (req: Request, res: Response): Promise<void> => {
  ...
  let coverImage = '';
  if (isDataUrl) {
    const result = (await cloudinaryService.uploadImage(req.body.coverImage)) as UploadApiResponse;
    if (!result.public_id) {
