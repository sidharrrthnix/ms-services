import { cloudinaryService } from '@gig-service/cloudinary';
import { getDocumentCount } from '@gig-service/elasticsearch';
import { gigCreateSchema } from '@gig-service/schemes/gig';
import { createGig } from '@gig-service/services/gig.service';
import { BadRequestError, ISellerGig } from '@sidharrrthnix/ms-shared-package';
...
const gigCreate = async (req: Request, res: Response): Promise<void> => {
  ...
  const result = await cloudinaryService.uploadImage(req.body.coverImage);
  if (!result.public_id) {
